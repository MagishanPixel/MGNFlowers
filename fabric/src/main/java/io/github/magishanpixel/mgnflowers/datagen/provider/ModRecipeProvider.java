            ...
            public void buildRecipes() {
                if (MGNFlowers.dataGenList != null) {
                    for (Map.Entry<String, FlowerProperty> entry : MGNFlowers.dataGenList.entrySet()) {
                        ...
                        Item flower = BuiltInRegistries.ITEM.getValue(MGNFlowers.newId(entry.getKey()));

                        shapeless(RecipeCategory.MISC, itemDye)
                                .requires(flower)
                                .unlockedBy("has_" + entry.getKey(), has(flower))
                                .save(output, BuiltInRegistries.ITEM.getKey(itemDye).getPath() + "_from_" + entry.getKey());
                        ;
                    }
                }
            }
