...
public class ModWorldGen {
    public interface ModifyBiome {
        void run(ResourceLocation modifierId, ResourceKey<Biome> biome, ModKeyFeatures.KeyFeature keys);
    }

    public interface ModifyBiomeAsTag {
        void run(ResourceLocation modifierId, TagKey<Biome> biome, ModKeyFeatures.KeyFeature keys);
    }

    public static void boot(ModifyBiome v, ModifyBiomeAsTag vAsTag) {
        //worldGen.addFeatureToBiomes(matches(ModTags.IS_PLAINS), GenerationStep.Decoration.VEGETAL_DECORATION, ModKeyFeatures.TWIN_POPPY_PATCH.config().location());

        //vAsTag.run(MGNFlowers.newId("plains_flowers"), ModTags.IS_PLAINS, ModKeyFeatures.TWIN_POPPY_PATCH);
